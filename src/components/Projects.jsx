...
function ProjectCard({ project, index }) {
  return (
    <motion.div
      className={`bg-surface border border-border rounded-lg p-6 flex flex-col hover:border-accent/50 transition-colors duration-200 group ${
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        {project.tech.map((t) => (
          <span
            key={t}
            className="font-mono text-xs text-accent border border-accent/30 px-2 py-0.5 rounded"
          >
            {t}
          </span>
        ))}
